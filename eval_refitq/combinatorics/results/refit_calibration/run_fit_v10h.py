"""v10h = HYBRID seed: reported TTTrack C per track + per-bin PSD deficit. v10s = v10 with the seed also binned by OT stub count (4, 5, 6+; seed_structure_by_stubs.log). v10: EMPIRICAL seed covariance. The TTTrack covariance reports corr(rInv, phi0)
...
json.dump({"ml_material_scale": float(res.x), "nll": float(res.fun), "pt_bins": PTB,
           "seed": {f"{g},{ns},{b}": v.tolist() for (g, ns, b), v in E.items()}}, open(base + "fit_v10h.json", "w"), indent=1)
