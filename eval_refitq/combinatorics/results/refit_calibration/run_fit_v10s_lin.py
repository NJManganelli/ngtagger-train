"""v10s = v10 with the seed also binned by OT stub count (4, 5, 6+; seed_structure_by_stubs.log). v10 on LINEARISED residuals (see the block after dres). v10: EMPIRICAL seed covariance. The TTTrack covariance reports corr(rInv, phi0)
...
def with_empirical_seed(P):
    ...
    P2 = dict(P)
    P2["C"] = np.stack([E[(g, ns, bb)] for g, ns, bb in zip(P["g"], P["nst"], b)])
    return P2
...
json.dump({"ml_material_scale": float(res.x), "nll": float(res.fun), "pt_bins": PTB,
           "seed": {f"{g},{ns},{b}": v.tolist() for (g, ns, b), v in E.items()}}, open(base + "fit_v10s_lin.json", "w"), indent=1)
