"""v10h = HYBRID seed: reported TTTrack C per track + per-bin PSD deficit. v10s = v10 with the seed also binned by OT stub count (4, 5, 6+; seed_structure_by_stubs.log). v10 on LINEARISED residuals (see the block after dres). v10: EMPIRICAL seed covariance. The TTTrack covariance reports corr(rInv, phi0)
...
def pack(sel):
    ...
    P["ids"] = ut[kept]
    P["nst"] = np.clip(D["n_stubs"][first][kept], 4, 6) - 4          # 4, 5, 6+ stubs
...
for g in range(F.N_GROUP):
  for ns in range(3):
    for b in range(len(PTB) - 1):
        m = (Pall["g"] == g) & (Pall["nst"] == ns) & (pt_all >= PTB[b]) & (pt_all < PTB[b + 1])
        x = dres[Pall["ids"][m]]
        if m.sum() < 150:
            E[(g, ns, b)] = None
            continue
        ...
        # HYBRID: keep each track's reported covariance (its per-track position-direction
        # structure) and add only the per-bin correlated deficit
        Emp = np.cov(x[keep].T)
        Ce = Emp - Kall[m][keep].mean(0) - Pall["C"][m][keep].mean(0)
        ...
        # swamp rInv
        # normalise by the EMPIRICAL diagonal: the deficit's own diagonal can be ~0 or
        # negative after the subtraction, and dividing by it blows the projection up
        sd = np.sqrt(np.diag(Emp))
        w, V = np.linalg.eigh(Ce / np.outer(sd, sd))
        Ce = ((V * np.maximum(w, 0.0)) @ V.T) * np.outer(sd, sd)          # deficit: PSD part only
        E[(g, ns, b)] = Ce
        Cr = np.median(Pall["C"][m], 0)
        print(f"  {F.FLOOR_GROUPS[g][0]:18s} {ns+4}{'+' if ns==2 else ''} stubs pT [{PTB[b]:g},{PTB[b+1]:g}) n {m.sum():6d}: "
...
for (g, ns, b), v in list(E.items()):
    if v is None:
        near = sorted((abs(nn - ns) * 10 + abs(bb - b), nn, bb) for (gg, nn, bb), vv in E.items() if gg == g and vv is not None)
        E[(g, ns, b)] = E[(g, near[0][1], near[0][2])]
...
def with_empirical_seed(P):
    ...
    P2 = dict(P)
    P2["C"] = P["C"] + np.stack([E[(g, ns, bb)] for g, ns, bb in zip(P["g"], P["nst"], b)])
    return P2
...
json.dump({"ml_material_scale": float(res.x), "nll": float(res.fun), "pt_bins": PTB,
           "seed": {f"{g},{ns},{b}": v.tolist() for (g, ns, b), v in E.items()}}, open(base + "fit_v10h_lin.json", "w"), indent=1)
